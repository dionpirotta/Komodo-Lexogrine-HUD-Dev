...
class MapsBar extends React.PureComponent<Props> {
  render() {
    ...
    if (picks.length > 3) {
      ...
      return <div id="maps_container">{<MapEntry veto={current} map={map} logo={current.type === "decider" ? null : map.team_ct.id === current.teamId ? map.team_ct.logo : map.team_t.logo} />}</div>;
    }
    return (
      <div id="maps_container">
        {match.vetos
          ...
          .map((veto) => (
            <MapEntry key={veto.mapName} veto={veto} map={this.props.map} logo={veto.type === "decider" ? null : map.team_ct.id === veto.teamId ? map.team_ct.logo : map.team_t.logo} />
          ))}
      </div>
    );
  }
  ...
  render() {
    ...
    return (
      <div className="veto_entry">
        ...
        <div className={`map_name ${map.name.includes(veto.mapName) ? "active" : ""}`}>{veto.mapName.split("de_")}</div>
      </div>
    );
  }
}
