...
export default class AlertsBar extends React.Component<Props, State> {
	...
	modAlert = (
		text: string,
		side: 'left' | 'right',
		alert: 'roundWon' | 'planting' | 'defusing' | 'planted' | 'teamTimeout',
		time?: number
	) => {
		this.setState(
			state => {
	...
	componentDidMount() {
		...
		GSI.on('bombPlantStart', player => {
			...
			if (this.props.phase.phase !== 'over') {
				this.modAlert(
					`${player.name} is planting`,
					player.team.orientation,
					'planting'
				)
			}
		})
		...
		GSI.on('defuseStart', player => {
			...
			this.modAlert(
				`${player.name} is defusing${
					player.state.defusekit ? ' with a kit' : ''
				}`,
				player.team.orientation,
				'defusing'
			)
		...
		GSI.on('data', data => {
			if (
				this.state.right.alertType.planting ||
				this.state.left.alertType.planting
			) {
			...
			if (
				this.state.left.alertType.roundWon ||
				this.state.right.alertType.roundWon
			) {
				if (this.props.phase.phase !== 'over') {
			...
			if (
				data.phase_countdowns.phase === 'timeout_ct' ||
				data.phase_countdowns.phase === 'timeout_t'
			) {
				this.modAlert(
					`TIMEOUTS REMAINING: ${data.map.team_ct.timeouts_remaining}`,
					data.map.team_ct.orientation,
					'teamTimeout'
				)
				this.modAlert(
					`TIMEOUTS REMAINING: ${data.map.team_t.timeouts_remaining}`,
					data.map.team_t.orientation,
					'teamTimeout'
				)
			}
			if (
				this.state.left.alertType.teamTimeout ||
				this.state.right.alertType.teamTimeout
			) {
				if (
					this.props.phase.phase !== 'timeout_ct' &&
					this.props.phase.phase !== 'timeout_t'
				) {
	...
	render() {
		...
		const amountOfMaps =
			Math.floor(Number(match?.matchType.substr(-1)) / 2) + 1 || 0
		...
		return (
			<div className='alerts'>
				<div className={`side_box left`}>
					<TextAlert
						team={left}
						show={this.state.left.alertType.teamTimeout}
						text={this.state.left.text}
					/>
					<Bombarino
						team={left}
						show={this.state.left.alertType.planted}
						defusing={this.state.right.alertType.defusing}
					/>
					<TextAlert
						team={left}
						show={this.state.left.alertType.defusing}
						text={this.state.left.text}
					/>
					<TextAlert
						team={left}
						show={this.state.left.alertType.planting}
						text={this.state.left.text}
					/>
					<TextAlert
						team={left}
						show={this.state.left.alertType.roundWon}
						text={this.state.left.text}
					/>
					<SeriesScore
						team={left}
						show={this.state.left.seriesScore}
						wonmaps={amountOfMaps}
						wins={left_wins}
					/>
				</div>
				...
				<div className={`side_box right`}>
					<SeriesScore
						team={right}
						show={this.state.right.seriesScore}
						wonmaps={amountOfMaps}
						wins={right_wins}
					/>
					<TextAlert
						team={right}
						show={this.state.right.alertType.roundWon}
						text={this.state.right.text}
					/>
					<TextAlert
						team={right}
						show={this.state.right.alertType.planting}
						text={this.state.right.text}
					/>
					<TextAlert
						team={right}
						show={this.state.right.alertType.defusing}
						text={this.state.right.text}
					/>
					<Bombarino
						team={right}
						show={this.state.right.alertType.planted}
						defusing={this.state.left.alertType.defusing}
					/>
					<TextAlert
						team={right}
						show={this.state.right.alertType.teamTimeout}
						text={this.state.right.text}
					/>
				</div>
			</div>
