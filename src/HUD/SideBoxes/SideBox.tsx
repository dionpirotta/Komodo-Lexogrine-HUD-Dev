...
export default class SideBox extends React.Component<{ side: "left" | "right"; hide: boolean }, { title: string; subtitle: string; image?: string }> {
  ...
  render() {
    if (this.state.title == "hide") return "";
    return (
      <div className={`sidebox ${this.props.side} ${this.props.hide ? "hide" : "show"}`}>
        <div className={`title_container ${this.state.title == "image" ? "rect" : "square"}`}>
          <div className="title">{this.state.title}</div>
          <div className="subtitle">{this.state.subtitle}</div>
        </div>
        <div className={`image_container ${this.state.title == "image" ? "rect" : "square"}`}>
