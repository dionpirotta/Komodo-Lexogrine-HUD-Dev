...
class GrenadeContainer extends React.PureComponent<{ grenade: string; amount: number; team: string }> {
  render() {
    return (
      <div className="grenade_container">
        <div className={`grenade_image ${this.props.team}`}>
...
export default class SideBox extends React.Component<Props> {
  render() {
    const grenades = summarise(this.props.players, this.props.team);
    const total = Object.values(grenades).reduce((a, b) => a + b, 0);
    return (
      <div className={`utilitybox ${this.props.team || ""} ${this.props.show ? "show" : "hide"}`}>
        ...
        <div className={`grenades_container ${this.props.side}`}>
          <GrenadeContainer grenade="smokegrenade" amount={grenades.smokes} team={this.props.team} />
          <GrenadeContainer grenade={this.props.team === "CT" ? "incgrenade" : "molotov"} amount={grenades.inc} team={this.props.team} />
          <GrenadeContainer grenade="flashbang" amount={grenades.flashes} team={this.props.team} />
          <GrenadeContainer grenade="hegrenade" amount={grenades.hg} team={this.props.team} />
        </div>
      </div>
    );
  }
